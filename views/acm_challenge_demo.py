...
def get_segmented_videos (request, central_video_input_id = 4, chosen_segment_id = 0):
    ...
    all_videos = json.loads(open(settings.SERVER_PATH+"data/acm_data.json").read())
    nodes = []
...
#remove_count..
    ...
    url_file = open(settings.SERVER_PATH + "data/videos_urls.txt", "r")
...
#print central_video['thumbnail']
    ...
    nodes.append({"group":central_video_group, 
                  ...
                  "word_cloud":"media/"+str(central_video['word_cloud']), 
                  "snapshot":"media/"+ str(central_video['thumbnail']).split("/")[0]+"/"+str(central_video['thumbnail']).split("/")[1]+"/thumbnail_8.jpg",
...
#            print "added " +  str(video['video_id'])
            ...
            nodes.append({"duration":video_duration[video['video_id']], "title":titles[video['video_id']],"slides":all_slides_per_video[video['video_id']], "id": video_node_id, 'url':video_urls['video_'+str(video['video_id'])], "group":peripheral_video_group, "image_width": peripheral_video_size * image_ratio[0], "image_height":peripheral_video_size * image_ratio[1], "size": peripheral_video_size, "radius": peripheral_video_radius, "cloud_width": cloud_size_video[0], "cloud_height": cloud_size_video[1], "word_cloud":"media/"+str(video['word_cloud']) , "snapshot":"media/"+str(video['thumbnail']).split("/")[0]+"/"+str(video['thumbnail']).split("/")[1]+"/thumbnail_8.jpg", "name":"video"+ str(video['video_id'])})
...
#                print "ENTERED for " + str(related_segment_id)
                ...
                child_node = {"group":segment_group, "size": peripheral_segment_size, "id":    related_segment_node_id, "image_width":  peripheral_segment_size * image_ratio_segment[0], "image_height": peripheral_segment_size * image_ratio_segment[1], "cloud_width": cloud_size[0], "cloud_height": cloud_size[1],"word_cloud":"media"+str(related_segment['word_cloud']) , "snapshot":"media/"+str(related_segment['thumbnail']), "name":"segment"+str(related_segment_id)}
...
#                print "parent_id "+str(parent_id) + "for " + str(related_segment_id)
                parent_node = {"group":peripheral_video_group, "size": peripheral_video_size, "radius": peripheral_video_radius, "image_width": peripheral_video_size * image_ratio[0], "image_height":peripheral_video_size * image_ratio[1], "cloud_width": cloud_size[0], "cloud_height": cloud_size[1], "word_cloud":"media"+str(parent['word_cloud']), "snapshot":"media/"+ str(parent['thumbnail']).split("/")[0]+"/"+str(parent['thumbnail']).split("/")[1]+"/thumbnail_8.jpg", "name":"video"+ str(parent['video_id'])}
...
#        print "segment " + str(segment_id) + "for parent node " + str(video['video_id']) 
        ...
        new_node = {"group":segment_group, "size": segment_size, "image_width": segment_size * image_ratio[0], "image_height":  segment_size * image_ratio[1], "parent": video_node_id, "id":len(nodes), "cloud_width": cloud_size[0], "cloud_height": cloud_size[1], "word_cloud":"media/"+str(segment['word_cloud']), "snapshot":"media/"+str(segment['thumbnail']), "name":"segment"+str(segment_id)}
...
def extract_title(folder_name):
    path=settings.SERVER_PATH+"media/om_final_Video_with_slides_with_all_features_Big_segments_20130619/"+folder_name
    if os.path.exists(path)==False:
        path=settings.SERVER_PATH+"media/om_final_Video_with_subtitles_with_all_features_Big_segments_20130619/"+folder_name
        if os.path.exists(path)==False:
            return ""
...
def extract_slides(folder_name):
    ...
    for f in os.listdir(path):
        ...
        #    print "-"*100
            ...
            for line_number, line in enumerate(open(Meta_data_filename,'r')):
                ...
                if Slid_flg_start == True:
                    ...
                    if line.rstrip()!="Slides":
                        ...
                        image_path = "media/om_final_Video_with_slides_with_all_features_Big_segments_20130619/"+ folder_name + "/images"
            ...
            input = PdfFileReader(file('media/om_final_Video_with_slides_with_all_features_Big_segments_20130619/kolokviji_ekaykin_drilling/sduqoir3au4ug4gzpj6kd56en7vz3x3c.pdf', "rb"))    
